...
import { isZipFile, decryptOfficeFile } from '../src/ooxml'
...
describe('Decrypting Password encrypted MS Office file', () => {
  ...
  it('Ole file (Word) with right password', async function () {
    ...
    assert.strictEqual(isZipFile(input), false)
    const output = await decryptOfficeFile(input, () => 'testtest')
    assert.strictEqual(!isZipFile(output), false)
  })
  ...
  it('Ole file (Excel) with right password', async function () {
    ...
    assert.strictEqual(isZipFile(input), false)
    const output = await decryptOfficeFile(input, () => 'test')
    assert.strictEqual(!isZipFile(output), false)
  })
  ...
  it('Ole file (Word) with wrong password', async function () {
    ...
    const input = jetpack.read(wordDoc, 'buffer')
    assert.strictEqual(isZipFile(input), false)
    await expect(decryptOfficeFile(input, () => 'testtes')).to.be.rejectedWith(Error).and.eventually.satisfy(error => {
      assert.strictEqual(error.id, 'GO_NOT_A_ZIPFILE')
  ...
  it('Not an Ole file', async function () {
    ...
    assert.strictEqual(!isZipFile(input), false)
    await expect(decryptOfficeFile(input, () => 'testtest')).to.be.rejectedWith(Error).and.eventually.satisfy(error => {
      assert.strictEqual(error.id, 'GO_NOT_A_COMPOUND_FILE')
